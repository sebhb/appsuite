...
class AppointmentGenerator {
        ...
        for template in appointmentDesciptions {
            ...
            if (minGuests != nil) || (maxGuests != nil) {
                ...
                if !guests.isEmpty {
                    ...
                    let combinedNames = formatList(items: firstNames, locale: locale)
    ...
    private func formatList(items: [String], locale: Locale? = nil) -> String {
#if os(macOS)
        let listFormater = ListFormatter()
        listFormater.locale = locale
        let list = listFormater.string(from: items)!
        // Use native ListFormatter
        return list
#else
        // Fallback for Linux -- ignoring the locale at the moment
        switch items.count {
            case 0: return ""
            case 1: return items[0]
            case 2: return "\(items[0]) and \(items[1])"
            default:
                let allButLast = items[0..<items.count-1].joined(separator: ", ")
